...
async def root():
    ...
    return {"service": "photogrammetry", "status": "running", "version": "1.0.0"}


@app.get("/health")
async def health_check():
    """Detailed health check."""
    colmap_path = shutil.which(settings.colmap_bin)
    return {
        "status": "healthy" if colmap_path else "degraded",
        ...
        "supported_output_formats": ["gaussian-splatting"],
    }
...
async def start_scan(request: ScanRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при запуске сканирования: {str(e)}"
        )
...
async def get_model(model_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении модели: {str(e)}"
        )
