...
const styleAnalysisArbitrary = fc.record({
	...
	styleTags: fc.array(fc.string({ minLength: 3, maxLength: 20 }), {
		minLength: 1,
		maxLength: 5,
	}),
	analyzedAt: fc.date(),
...
const modificationTypesArbitrary = fc.constant([
	"recolor",
	"restoration",
	"geometry",
] as const);
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async (
					...
					modificationTypes: readonly string[],
				) => {
					const suggestions: ModificationSuggestion[] = modificationTypes.map(
						(type, index) => {
							if (type === "recolor") {
								return {
									id: `suggestion-${index}`,
					...
					const suggestionTypes = new Set(
						suggestions.map((s) => s.modificationType),
					);
					...
					suggestions.forEach((suggestion) => {
						if (suggestion.modificationType === "recolor") {
							...
							expect(
								Object.keys(suggestion.parameters.color_map).length,
							).toBeGreaterThan(0);
						} else if (suggestion.modificationType === "restoration") {
							expect(Array.isArray(suggestion.parameters.damaged_regions)).toBe(
								true,
							);
							expect(
								suggestion.parameters.damaged_regions.length,
							).toBeGreaterThan(0);
							expect(suggestion.parameters.restoration_style).toBeTruthy();
						} else if (suggestion.modificationType === "geometry") {
							expect(
								suggestion.parameters.modification_description,
							).toBeTruthy();
						}
					});
				},
			...
			fc.asyncProperty(
				...
				async (styleAnalysis: StyleAnalysis) => {
					...
					if (suggestion.parameters.finish) {
						expect(["matte", "glossy", "satin"]).toContain(
							suggestion.parameters.finish,
						);
					}
				},
			),
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async (styleAnalysis: StyleAnalysis) => {
					...
					expect(suggestion.parameters.damaged_regions.length).toBeGreaterThan(
						0,
					);
					expect(suggestion.parameters.restoration_style).toBeTruthy();
				},
			...
			fc.asyncProperty(
				...
				async (styleAnalysis: StyleAnalysis) => {
					...
					if (suggestion.parameters.scale_factor) {
						expect(suggestion.parameters.scale_factor).toBeGreaterThanOrEqual(
							0.1,
						);
						expect(suggestion.parameters.scale_factor).toBeLessThanOrEqual(
							10.0,
						);
					}
					...
					if (suggestion.parameters.dimensions) {
						...
						Object.values(suggestion.parameters.dimensions).forEach(
							(value: any) => {
