...
async function createModificationSuggestion(modelId: string) {
	...
	if (error) {
		throw new Error(
			`Failed to create modification suggestion: ${error.message}`,
		);
	}
...
async function checkModificationSuggestionsExist(
	modelId: string,
): Promise<boolean> {
	if (!supabaseAdmin) {
		return false;
	...
	if (error) {
		throw new Error(
			`Failed to check modification suggestions: ${error.message}`,
		);
	}
		...
		await fc.assert(
			fc.asyncProperty(
				fc.record({
					...
					modelType: fc.constantFrom(
						"gaussian-splatting" as const,
						"nerf" as const,
					),
				}),
				async ({ email, projectName, photoCount, modelType }) => {
					...
					try {
						...
						expect(await checkModificationSuggestionsExist(modelId!)).toBe(
							true,
						);
						...
						const deletedProjectResponse = supabaseAdmin
							? await supabaseAdmin
									.from("projects")
									.select("id")
									.eq("id", projectId)
									.single()
							: { data: null };
						const { data: deletedProject } = deletedProjectResponse;
