...
async function simulateProcessingStart(
	projectId: string,
	_uploadCompletedAt: Date,
...
function validateProcessingStartTime(
	uploadCompletedAt: Date,
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async ({ projectId, photoCount }) => {
					...
					const processingJob = await simulateProcessingStart(
						projectId,
					...
					const timeDifference = Math.abs(
						processingJob.startedAt.getTime() -
							processingJob.createdAt.getTime(),
					);
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async ({ projectId, photoCount, systemDelayMs }) => {
					...
					const isValid = validateProcessingStartTime(
						uploadResult.uploadCompletedAt,
						processingStartTime,
					);
					const actualDelay =
						processingStartTime.getTime() -
						uploadResult.uploadCompletedAt.getTime();
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async ({ projectId, photoCount }) => {
					...
					expect(
						uploadResult.uploadCompletedAt.getTime(),
					).toBeGreaterThanOrEqual(testStartTime.getTime());
					expect(processingJob.startedAt.getTime()).toBeGreaterThanOrEqual(
						uploadResult.uploadCompletedAt.getTime(),
					);
					expect(processingJob.startedAt.getTime()).toBeLessThanOrEqual(
						testEndTime.getTime(),
					);
