...
import { ComparisonView, Model3DViewer } from "@/components/3d";
import { MaterialSpecification } from "@/components/modifications/MaterialSpecification";
import {
	type PhotoUploadError,
	PhotoUploader,
} from "@/components/photos/PhotoUploader";
...
function getModelVersionLabel(model: ModelResponse, index: number) {
	const versionName = model.is_original ? "Оригинал" : `Версия ${index + 1}`;
	const createdAt = new Date(model.created_at).toLocaleDateString("ru-RU");

	return `${versionName} - ${createdAt}`;
}
...
export default function ProjectPage({ params }: ProjectPageProps) {
	...
	const [latestModification, setLatestModification] =
		useState<ModificationSummary | null>(null);
	const [latestModel, setLatestModel] = useState<ModelResponse | null>(null);
	const [isComparisonMode, setIsComparisonMode] = useState(false);
	const [leftModelId, setLeftModelId] = useState<string | null>(null);
	...
	const fetchModels = useCallback(async () => {
		...
		setLatestModification(payload.latestModification || null);
		setModelError(null);
	}, [projectId]);
	...
	useEffect(() => {
		...
		setRightModelId((current) => {
			if (current && models.some((model) => model.id === current)) {
				return current;
			}

			return models[1]?.id ?? models[0]?.id ?? null;
		});
	}, [models]);
	...
	const selectedLeftModel = useMemo(
		() => models.find((model) => model.id === leftModelId) ?? null,
		[leftModelId, models],
	);
	const selectedRightModel = useMemo(
		() => models.find((model) => model.id === rightModelId) ?? null,
		[models, rightModelId],
	);
	const canCompare = models.length >= 2;
	const hasDistinctComparisonSelection = Boolean(
		selectedLeftModel &&
			selectedRightModel &&
			selectedLeftModel.id !== selectedRightModel.id,
	);
	const showComparison = Boolean(
		isComparisonMode && canCompare && hasDistinctComparisonSelection,
	);
	const specificationModificationId = latestModification?.id ?? null;

	useEffect(() => {
		if (isComparisonMode && !hasDistinctComparisonSelection) {
			setIsComparisonMode(false);
		}
	}, [hasDistinctComparisonSelection, isComparisonMode]);
	...
	const handleUploadComplete = useCallback(async () => {
		setUploadError(null);
		setScanMessage(
			"Фотографии загружены. Теперь можно запускать сканирование.",
		);
			...
			<div className="mx-auto max-w-6xl space-y-6">
				<section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
							...
							<div className="space-y-3">
								...
								<div className="space-y-2">
									...
									<p className="max-w-3xl text-sm leading-6 text-slate-600">
										{project.description ||
											"Добавьте фотографии предмета, проверьте количество кадров и запустите сборку 3D-модели."}
									</p>
								</div>
							</div>
					...
					<div className="grid gap-6 px-6 py-6 sm:px-8 lg:grid-cols-[1.1fr_0.9fr]">
						<section className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50/70 p-5">
							<div className="flex items-center justify-between gap-4">
								<div>
									...
									<p className="text-sm text-slate-500">
										Загрузите минимум 10 кадров с разных ракурсов.
									</p>
								</div>
							...
							{uploadError && (
								<div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
									<p className="font-medium">{uploadError.userMessage}</p>
									{uploadError.recommendations &&
										uploadError.recommendations.length > 0 && (
						...
						<section className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5">
							<div>
								<p className="text-sm font-semibold text-slate-900">
									Сканирование и прогресс
								</p>
								<p className="text-sm text-slate-500">
									Как только кадров достаточно, можно запускать реконструкцию
									модели.
								</p>
							</div>
							...
							{jobLoading && !job ? (
								...
								<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-sm text-slate-500">
									После запуска сканирования здесь появится индикатор прогресса
									и расчет времени.
								</div>
							)}
							...
							<div className="rounded-2xl border border-slate-200 bg-[linear-gradient(135deg,#fff8eb_0%,#fffefe_100%)] p-4 text-sm text-slate-600">
								...
								<p className="mt-2">
									Когда задача завершится, проект перейдет в статус{" "}
									<span className="font-medium text-emerald-700">готов</span>.
									Следующим шагом можно будет подключить просмотр модели и
									дальнейший анализ.
								</p>
							</div>
						</section>
				...
				<section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
						<div>
							...
							<p className="text-sm text-slate-500">
								После завершения сканирования здесь появляется текущая версия
								модели и режим сравнения версий.
							</p>
						</div>
						{latestModel && (
							<p className="text-sm text-slate-400">
								Последняя модель:{" "}
								{new Date(latestModel.created_at).toLocaleString("ru-RU")}
							</p>
						)}
					</div>
					...
					<div className="mt-6 space-y-4">
						<div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 lg:flex-row lg:items-end lg:justify-between">
							<div>
								...
								<p className="mt-1 text-sm text-slate-500">
									Выберите две версии модели, чтобы синхронно сравнивать ракурсы
									и изменения.
								</p>
							</div>
							...
							<div className="flex flex-col gap-3 sm:flex-row sm:items-end">
								<label className="space-y-2 text-sm text-slate-600">
									...
									<select
										...
										disabled={!canCompare}
										className="min-w-[220px] rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-500"
									>
										{models.map((model, index) => (
											<option key={`left-${model.id}`} value={model.id}>
												{getModelVersionLabel(model, index)}
											</option>
										))}
									</select>
								</label>
								...
								<label className="space-y-2 text-sm text-slate-600">
									...
									>
										{models.map((model, index) => (
											<option key={`right-${model.id}`} value={model.id}>
												{getModelVersionLabel(model, index)}
											</option>
										))}
									</select>
								...
								<Button
									...
									disabled={!canCompare || !hasDistinctComparisonSelection}
									onClick={() => setIsComparisonMode((current) => !current)}
								>
									{isComparisonMode ? "Выйти из сравнения" : "Сравнить версии"}
								</Button>
							</div>
						</div>

						{!canCompare && latestModel && (
							<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm text-slate-500">
								Для сравнения нужны минимум две версии модели. Сейчас доступна
								только одна запись в models_3d.
							</div>
						)}

						{canCompare && !hasDistinctComparisonSelection && (
							<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
								Выберите две разные версии модели для синхронного сравнения.
							</div>
						)}
						...
						{showComparison && selectedLeftModel && selectedRightModel ? (
							<ComparisonView
								leftModel={selectedLeftModel}
								rightModel={selectedRightModel}
							...
							<div className="rounded-2xl border border-dashed border-amber-300 bg-amber-50 px-6 py-10 text-center text-sm text-amber-800">
								Модель готовится. Как только сканирование завершится,
								просмотрщик появится автоматически.
							</div>
						) : project.status === "ready" ? (
							<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
								Проект уже помечен как готовый, но модель пока не найдена.
								Обновите страницу или проверьте запись в models_3d.
							</div>
						) : (
							<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
								Сначала загрузите фотографии и завершите сканирование, чтобы
								увидеть 3D-предпросмотр объекта.
							</div>
						)}
					</div>
				</section>
				...
				<section className="space-y-4">
					<div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
						<div>
							<h2 className="text-lg font-semibold text-slate-900">
								Материалы для модификации
							</h2>
							<p className="text-sm text-slate-500">
								Последняя завершенная модификация получает список материалов и
								инструкции для мастерской.
							</p>
						</div>
						{latestModification && (
							<p className="text-sm text-slate-400">
								{latestModification.modificationType} -{" "}
								{latestModification.status}
							</p>
						)}
					</div>
					<MaterialSpecification modificationId={specificationModificationId} />
				</section>
				...
				<section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
						<div>
							...
							<p className="text-sm text-slate-500">
								Все фотографии проекта, которые уже попали в хранилище.
							</p>
						</div>
						<p className="text-sm text-slate-400">
								...
								>
									...
									<div className="space-y-1 px-3 py-3">
										<p className="text-xs font-medium text-slate-700">
											{photo.size_bytes
												? `${(photo.size_bytes / 1024 / 1024).toFixed(1)} МБ`
												: "Размер неизвестен"}
										</p>
