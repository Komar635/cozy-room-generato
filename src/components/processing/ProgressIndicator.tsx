...
import type { ProcessingJob } from "@/hooks/useProcessingJobStatus";
	...
	return (
		<div className={`space-y-4 ${className}`}>
					...
					showExecutionTime && (
						<span className="text-sm text-gray-500">
							{job.status === "processing"
								...
								: `Выполнено за ${formatTime(displayElapsedTime)}`}
						</span>
					)}
			</div>
			...
			<div className="flex items-center justify-between text-sm">
				<span className="text-gray-600">{job.progress}% завершено</span>
