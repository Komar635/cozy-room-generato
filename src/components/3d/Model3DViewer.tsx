...
import type { OrbitViewState } from "./Scene3D";
...
function Model3DContent({
	...
	if (modelType === "gaussian-splatting") {
		...
		return <NeRFLoader url={modelUrl} onLoad={onLoad} onError={onError} />;
	}

	onError?.(new Error(`Unsupported 3D model type: ${modelType}`));
	return null;
}
				...
				<div className="text-center p-4">
					<div className="text-red-500 mb-2">
						<svg
							...
							viewBox="0 0 24 24"
							role="img"
							aria-label="Ошибка загрузки модели"
						>
							<title>Ошибка загрузки модели</title>
							<path
								strokeLinecap="round"
