...
class MockOrbitControls {
	...
	constructor(
		...
		initialCameraPosition: [number, number, number] = [0, 0, 5],
	) {
		this.config = config;
		this.camera = {
...
const _viewerConfigArbitrary = fc.record({
	modelUrl: fc.webUrl(),
...
describe("Feature: reality-digitizer-3d, Property 19: Интерактивность визуализатора", () => {
		...
		await fc.assert(
			fc.property(
				...
				(config, interaction) => {
					...
					fc.pre(
						config.minDistance < config.maxDistance && config.minDistance > 0,
					);
					...
					switch (interaction.type) {
						case "rotate":
							if (
								config.enableRotate &&
								(Math.abs(interaction.deltaX) >= 0.005 ||
									Math.abs(interaction.deltaY) >= 0.005)
							) {
								expect(interactionHandled).toBe(true);
								// Вращение должно изменить углы поворота
								const rotationChanged =
									Math.abs(finalState.rotation[0] - initialState.rotation[0]) >
										0.00001 ||
									Math.abs(finalState.rotation[1] - initialState.rotation[1]) >
										0.00001;
						...
						case "pan":
							if (
								...
								const positionChanged =
									Math.abs(finalState.position[0] - initialState.position[0]) >
										0.00001 ||
									Math.abs(finalState.position[1] - initialState.position[1]) >
										0.00001;
						...
						case "zoom":
							if (
								config.enableZoom &&
								Math.abs(interaction.deltaZoom) >= 0.005
							) {
								...
								expect(finalDistance).toBeGreaterThanOrEqual(
									config.minDistance,
								);
					...
					for (const value of finalState.position) {
						expect(Number.isFinite(value)).toBe(true);
					}
					for (const value of finalState.rotation) {
						expect(Number.isFinite(value)).toBe(true);
					}
				},
			),
	...
	it("should validate OrbitControls configuration correctly", async () => {
		await fc.assert(
			fc.property(orbitControlsConfigArbitrary, (config) => {
	...
	it("should handle edge cases correctly", async () => {
		await fc.assert(
			fc.property(orbitControlsConfigArbitrary, (config) => {
				...
				const smallInteractionHandled = controls.handleInteraction(
					"rotate",
					0.001,
					0.001,
				);
				...
				if (config.enableZoom) {
					...
					expect(minDistance).toBeGreaterThanOrEqual(
