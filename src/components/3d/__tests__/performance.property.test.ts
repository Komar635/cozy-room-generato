...
import {
	clearModelPerformanceCaches,
	createLODPlan,
	estimateModelCreationTime,
	getModelAssetCacheSize,
...
class MockRenderer {
	...
	simulateRender(modelComplexity: ModelComplexity): PerformanceMetrics {
		...
		const frameTime = Math.max(
			16.67,
			this.basePerformance.frameTime * totalFactor,
		); // Минимум 60 FPS
		...
		const memoryUsage =
			this.basePerformance.memoryUsage +
	...
	private calculateComplexityFactor(complexity: ModelComplexity): number {
		...
		const textureFactor =
			1 +
			((complexity.textureSize * complexity.textureSize) / (4096 * 4096)) * 0.3;
		const materialFactor = 1 + (complexity.materialCount / 20) * 0.1; // Ещё менее агрессивное влияние материалов

		return Math.min(
			2.0,
			vertexFactor * triangleFactor * textureFactor * materialFactor,
		...
		await fc.assert(
			fc.property(
				...
				(context, { simpleModel, complexModel }) => {
					...
					expect(complexMetrics.frameTime).toBeGreaterThanOrEqual(
						simpleMetrics.frameTime,
					);
					expect(complexMetrics.memoryUsage).toBeGreaterThanOrEqual(
						simpleMetrics.memoryUsage,
					);
			...
			fc.property(
				...
				(complexity, { width, height, pixelRatio }) => {
					...
					const lowQualityMetrics =
						lowQualityRenderer.simulateRender(complexity);
					const highQualityMetrics =
						highQualityRenderer.simulateRender(complexity);

					// Assert: высокое качество должно снижать производительность
					expect(highQualityMetrics.fps).toBeLessThanOrEqual(
						lowQualityMetrics.fps,
					);
					expect(highQualityMetrics.frameTime).toBeGreaterThanOrEqual(
						lowQualityMetrics.frameTime,
					);
		...
		await fc.assert(
			fc.property(
				...
				(context, complexity) => {
					...
					const expectedMemoryFromTextures =
						complexity.textureSize * complexity.textureSize * 0.0000003;
					const expectedTotalMemory =
						50 + expectedMemoryFromVertices + expectedMemoryFromTextures;
...
const modelCreationInputArbitrary = fc.record({
	photoCount: fc.integer({ min: 12, max: 120 }),
	averagePhotoMegapixels: fc.float({
		min: Math.fround(0.8),
		max: Math.fround(8),
		noNaN: true,
	}),
...
describe("Feature: reality-digitizer-3d, Property 16: Время создания 3D-модели", () => {
	it("keeps model creation estimate within the 10 minute requirement for valid scan inputs", async () => {
		await fc.assert(
			fc.asyncProperty(modelCreationInputArbitrary, async (input) => {
				const estimate = estimateModelCreationTime(input);

				expect(estimate.estimatedMs).toBeGreaterThan(0);
				expect(estimate.estimatedMs).toBeLessThanOrEqual(10 * 60 * 1000);
				expect(estimate.progressivePreviewMs).toBeLessThan(
				...
				expect(Number.isFinite(estimate.estimatedMs)).toBe(true);
				expect(Number.isFinite(estimate.compressedBytes)).toBe(true);
			}),
			{ numRuns: 100 },
		);
	});

	it("uses progressive compressed assets and cache for repeated viewer loads", async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.webUrl().filter((url) => !/\.(br|gz)$/i.test(url)),
				fc.constantFrom("gaussian-splatting", "nerf"),
				async (url, modelType) => {
					clearModelPerformanceCaches();
					const compressed = resolveCompressedModelUrl(`${url}/model.splat`);
					const progressEvents: number[] = [];

					const firstAsset = await loadProgressiveModelAsset(
						compressed.url,
						modelType,
						(progress) => progressEvents.push(progress),
					);
					const cacheSizeAfterFirstLoad = getModelAssetCacheSize();
					const secondAsset = await loadProgressiveModelAsset(
						compressed.url,
						modelType,
					);

					expect(compressed.compressed).toBe(true);
					expect(cacheSizeAfterFirstLoad).toBe(1);
					expect(getModelAssetCacheSize()).toBe(cacheSizeAfterFirstLoad);
					expect(secondAsset).toEqual(firstAsset);
					expect(progressEvents[0]).toBeGreaterThan(0);
					expect(progressEvents.at(-1)).toBe(1);
					expect(firstAsset.pointCount).toBeGreaterThan(0);
					expect(firstAsset.triangleBudget).toBeGreaterThan(0);
				},
			),
			{ numRuns: 50 },
		);
	...
	it("reduces LOD budgets for smaller viewports without disabling rendering", async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.constantFrom("gaussian-splatting", "nerf"),
				fc.integer({ min: 320 * 240, max: 1920 * 1080 }),
				async (modelType, viewportPixels) => {
					const mobilePlan = createLODPlan(modelType, viewportPixels);
					const desktopPlan = createLODPlan(modelType, 1920 * 1080);

					expect(mobilePlan.previewPointCount).toBeGreaterThan(0);
					expect(mobilePlan.fullPointCount).toBeGreaterThanOrEqual(
						mobilePlan.previewPointCount,
					);
					expect(mobilePlan.fullPointCount).toBeLessThanOrEqual(
						desktopPlan.fullPointCount,
					);
					expect(mobilePlan.triangleBudget).toBeLessThanOrEqual(
						desktopPlan.triangleBudget,
					);
				},
			),
			{ numRuns: 100 },
