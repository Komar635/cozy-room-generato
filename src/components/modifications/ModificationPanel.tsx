		...
		switch (type) {
				...
				return (
					<div className="space-y-2">
						...
						<ul className="list-disc list-inside text-sm text-gray-600">
							{(parameters.damaged_regions || []).map(
			...
			case "geometry":
				return (
					<div className="space-y-2">
						<p className="text-sm text-gray-600">
							{parameters.modification_description}
						</p>
						{parameters.scale_factor && (
							<p className="text-sm text-gray-600">
								Масштаб:{" "}
								<span className="font-medium">{parameters.scale_factor}x</span>
							</p>
						)}
				...
				<Card>
					<CardContent className="py-12">
						<p className="text-center text-gray-500">
							Нет доступных предложений. Запустите анализ стиля для генерации
							предложений.
						</p>
					</CardContent>
				</Card>
					...
					<Card key={suggestion.id} className="flex flex-col">
						<CardHeader>
							<div className="flex items-start justify-between">
								...
								<Badge
									className={
										ModificationTypeColors[suggestion.modificationType]
									}
									variant="secondary"
								>
